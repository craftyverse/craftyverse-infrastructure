...
resource "aws_instance" "jenkins_server" {
  ...
  key_name = aws_key_pair.craftyverse_jenkins_kp.key_name
...
# Creating a key pair in AWS called craftyverse_jenkins_kp
resource "aws_key_pair" "craftyverse_jenkins_kp" {
  # Naming the key craftyverse-jenkins-kp
  key_name = "craftyverse-jenkins-kp"

  # Passing the public key of the key pair we created
  public_key = file("${path.module}/craftyverse_jenkins_kp.pub")
}
